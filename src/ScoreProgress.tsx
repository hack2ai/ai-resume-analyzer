...
export default function ScoreProgress({analyses,dark=false}:Props){
 const items=useMemo(()=>[...analyses].sort((a,b)=>new Date(a.createdAt||0).getTime()-new Date(b.createdAt||0).getTime()).slice(-8),[analyses]);
 if(!items.length)return null;
 const latest=items[items.length-1];
 const previous=items.length>1?items[items.length-2]:null;
 const first=items[0];
 const best=Math.max(...items.map(x=>x.atsScore));
 const changeFromFirst=latest.atsScore-first.atsScore;
 const changeFromPrevious=previous?latest.atsScore-previous.atsScore:0;
 const matchChangeFromPrevious=previous?latest.matchPercentage-previous.matchPercentage:0;
 const max=Math.max(100,...items.map(x=>Math.max(x.atsScore,x.matchPercentage)));
 ...
 return <section style={{marginTop:24,padding:22,borderRadius:24,border:`1px solid ${border}`,background:bg,boxShadow:"0 18px 50px rgba(15,23,42,.06)"}}>
  <div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"center",flexWrap:"wrap"}}><div><div style={{display:"flex",alignItems:"center",gap:8,color:"#7c3aed"}}><TrendingUp size={20}/><strong>Score progress</strong></div><p style={{margin:"7px 0 0",color:muted}}>Track how your resume performance changes across analyses.</p></div><div style={{display:"inline-flex",alignItems:"center",gap:7,padding:"8px 11px",borderRadius:999,background:changeFromFirst>=0?"rgba(34,197,94,.12)":"rgba(239,68,68,.1)",color:changeFromFirst>=0?"#15803d":"#dc2626",fontWeight:800}}>{changeFromFirst>=0?<ArrowUpRight size={16}/>:<ArrowDownRight size={16}/>} {changeFromFirst>=0?"+":""}{changeFromFirst}% since first scan</div></div>
  ...
  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:10,marginTop:14}}><Delta label="ATS vs previous" value={changeFromPrevious}/><Delta label="Match vs previous" value={matchChangeFromPrevious}/></div>
  <div style={{marginTop:24,padding:18,borderRadius:18,background:dark?"rgba(30,41,59,.6)":"#f8fafc"}}><div style={{display:"flex",height:190,alignItems:"end",gap:10}}>{items.map((item,i)=><div key={item.id||item.analysisId||i} style={{flex:1,minWidth:24,height:"100%",display:"flex",alignItems:"end",gap:4}}><div title={`${item.atsScore}% ATS`} style={{height:`${Math.max(8,(item.atsScore/max)*100)}%`,flex:1,borderRadius:"10px 10px 3px 3px",background:"linear-gradient(180deg,#7c3aed,#2563eb)",transition:"height .25s ease"}}/><div title={`${item.matchPercentage}% Match`} style={{height:`${Math.max(8,(item.matchPercentage/max)*100)}%`,flex:1,borderRadius:"10px 10px 3px 3px",background:"rgba(34,197,94,.65)"}}/></div>)}</div><div style={{display:"flex",justifyContent:"space-between",marginTop:10,fontSize:12,color:muted}}><span>Oldest</span><span>ATS <span style={{opacity:.65}}>and</span> Job Match</span><span>Latest</span></div></div>
 </section>;
}
...
function Delta({label,value}:{label:string;value:number}){const positive=value>=0;return <div style={{padding:"12px 14px",borderRadius:14,background:positive?"rgba(34,197,94,.08)":"rgba(239,68,68,.08)"}}><div style={{fontSize:12,opacity:.65}}>{label}</div><strong style={{display:"block",marginTop:4,color:positive?"#15803d":"#dc2626"}}>{positive?"+":""}{value}%</strong></div>}
