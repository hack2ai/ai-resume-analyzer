...
function Panel({icon,title,children}:{icon:React.ReactNode;title:string;children:React.ReactNode}){return <div style={{padding:17,borderRadius:18,background:"rgba(148,163,184,.08)"}}><h3 style={{margin:"0 0 12px",fontSize:16,display:"flex",gap:8,alignItems:"center",color:"#7c3aed"}}>{icon}{title}</h3>{children}</div>}
function Muted({children}:{children:React.ReactNode}){return <p style={{margin:0,opacity:.6,lineHeight:1.65}}>{children}</p>}
const card=(dark:boolean):React.CSSProperties=>({marginTop:24,padding:22,borderRadius:24,border:`1px solid ${dark?"#334155":"#e2e8f0"}`,background:dark?"#0f172a":"#fff",boxShadow:"0 18px 50px rgba(15,23,42,.06)"});const eyebrow:React.CSSProperties={display:"inline-flex",alignItems:"center",gap:6,color:"#7c3aed",fontSize:12,fontWeight:800,textTransform:"uppercase",letterSpacing:".08em"};const chip:React.CSSProperties={padding:"7px 10px",borderRadius:999,border:"1px solid #ddd6fe",background:"rgba(255,255,255,.7)",fontSize:13,fontWeight:700};const empty:React.CSSProperties={minHeight:220,display:"grid",placeItems:"center",alignContent:"center",textAlign:"center",opacity:.65};
