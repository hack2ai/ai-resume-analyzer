...
app.post("/api/analyze", requireAuth, upload.single("resume"), async (req: AuthenticatedRequest, res) => {
 try {
  ...
  const parsed = await pdf(req.file.buffer); const resumeText = parsed.text.replace(/\s+/g, " ").trim();
  if (resumeText.length < 100) return res.status(400).json({ error: "The PDF does not contain enough readable resume text." });
  const response = await gemini.chat.completions.create({ model: process.env.GEMINI_MODEL || "gemini-3.7-flash", response_format: { type: "json_object" }, messages: [{ role: "system", content: "You are a precise resume and ATS analyst. Return only valid JSON with atsScore, matchPercentage, missingKeywords, strengths, improvements, and summary. Scores are estimates, not hiring decisions. Be practical and do not invent experience." }, { role: "user", content: `RESUME:\n${resumeText}\n\nTARGET JOB DESCRIPTION:\n${jobDescription}\n\nAnalyze the match and return the requested JSON.` }] });
